            ...
            createUserWithEmailAndPassword(auth, this.email, this.password)
                .then(() => {
                    ...
                    if (this.role == "bedrijf") {
                        add_to_collection("profiel_bedrijf", { UID: auth.currentUser.uid, hasProfile: false, filledInQuestionnaire : false, role: this.role, time: Timestamp.now() }, auth.currentUser.uid)
                    } else if (this.role == "cursist") {
                        add_to_collection("profiel_cursist", { UID: auth.currentUser.uid, hasProfile: false, filledInQuestionnaire : false, role: this.role, time: Timestamp.now() }, auth.currentUser.uid)
                    }
